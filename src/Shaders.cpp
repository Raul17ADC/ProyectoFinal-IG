...
void Shaders::setLight(const std::string& name, Light value) {
  ...
  glUniform1f(glGetUniformLocation(program, (name + ".innerCutOff").c_str()),
              glm::cos(glm::radians(value.innerCutOff)));
  glUniform1f(glGetUniformLocation(program, (name + ".outerCutOff").c_str()),
              glm::cos(glm::radians(value.outerCutOff)));
